...
class wf {
    ...
    homeActions() {
        eo(mf, t => {
            // Add both 'click' and 'touchstart' event listeners for cross-device compatibility
            t.addEventListener("click", () => {
                this.locomotive.scrollTo(vf); // Handles desktop click
            });
    
            t.addEventListener("touchstart", () => {
                this.locomotive.scrollTo(vf); // Handles mobile touch
            });
        });
    
        // Existing click handler for copying text
        yf.addEventListener("click", t => {
            ff(t); // Perform the copy functionality
            ua.textContent = "copied"; // Update text to show 'copied'
            setTimeout(() => {
                ua.textContent = "Click To Copy"; // Reset text after 2 seconds
